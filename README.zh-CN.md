...
**为什么会有这个警告？**  
CapsBar 已使用有效的 [Certum](https://www.certum.eu/) 代码签名证书进行签名（可验证发布者为 *Fangmeng Liu*）。但 Microsoft SmartScreen 的信任机制基于程序的**下载量和用户运行记录**来建立——新发布的软件即使持有有效证书，也需要一定时间积累信誉值。

注意，这里涉及**两套独立的信誉体系**：
- **浏览器下载拦截**（Chrome / Edge 下载拦截）—— 基于下载链接和文件哈希，逐渐消退速度较慢。
- **Windows SmartScreen 弹窗**（蓝色“Windows 已保护你的电脑”对话框）—— 基于全球用户的运行记录，只要足够多的用户运行过该文件，拦截很快就会消失。

你可能会发现，仅几次下载后 Windows 就不再弹窗了，而浏览器那边的拦截还会持续一段时间。随着用户增多，两种警告最终都会消失。
